...
def upsert_group_member(tg_group_id: int, user_id: int, full_name: str,
    ...
    """Upserts group + user + member row in a single atomic transaction."""
    full_name = (full_name or "").strip() or "Unknown"
    username  = (username  or "").strip() or None
    ...
    def _write():
        ...
        # Single transaction for all three upserts
...
def set_member_active(tg_group_id: int, user_id: int, full_name: str = None,
    ...
    def _write():
        ...
        cursor.execute(
            ...
            """,
            (user_id, (full_name or "").strip() or "Unknown", (username or "").strip() or None)
        )
        cursor.execute(
            """
