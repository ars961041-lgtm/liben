        ...
        "pages": [
            ...
            },
            {
                ...
                "content": (
                    ...
                    "🔧 <b>دالة مساعدة خفيفة:</b>\n"
                    "<code>get_target_user_id(message)</code>\n"
                    "في: <code>utils/helpers.py</code>\n"
                    "تُرجع: <code>user_id</code> فقط أو <code>None</code> — بدون رسائل خطأ\n\n"
                    "📨 <b>الإرسال الآمن:</b>\n"
                    "<code>safe_reply(message, text)</code>\n"
                    "في: <code>utils/helpers.py</code>\n"
                    "يتعامل مع حذف الرسالة الأصلية تلقائياً (fallback لـ send_message)\n\n"
                    "⚠️ <b>حالات الخطأ المعالجة:</b>\n"
                    "• @username غير موجود في DB → رسالة واضحة\n"
                    "• المستخدم مشرف → رفض مع رسالة (للكتم/حظر/تقييد)\n"
                    "• المستخدم ليس مشرفاً → رفض عند تنزيل المشرف\n"
                    "• المستخدم مؤسس → محمي من التنزيل\n"
            ...
            {
                "title": "🔧 لوحة الصلاحيات الموحدة — صلاحيات",
                "content": (
                    "📌 <b>الأمر:</b>\n"
                    ...
                    "🔄 <b>تدفق التنفيذ:</b>\n"
                    "1️⃣ <code>resolve_user()</code> → تحديد الهدف (رد / @username / ID)\n"
                    "2️⃣ فحص المُرسِل: مشرف + يملك <code>can_promote_members</code>\n"
                    "3️⃣ فحص البوت: مشرف + يملك <code>can_promote_members</code>\n"
                    "4️⃣ فحص الهدف: ليس مؤسساً، ليس مجهول الهوية، موجود في المجموعة\n"
                    "5️⃣ <code>bot.get_chat_member()</code> → تحميل الصلاحيات الحالية\n"
                    "6️⃣ <code>init_state()</code> → تهيئة الجلسة\n"
                    "7️⃣ <code>send_ui()</code> → عرض اللوحة\n"
                    "8️⃣ المشرف يضبط الصلاحيات أو يختار إعداداً مسبقاً\n"
                    "9️⃣ <code>apply_permissions()</code> → تطبيق عبر <code>promoteChatMember</code>\n"
                    "🔟 <code>_verify_applied()</code> → تحقق من <code>get_chat_member</code> وتسجيل أي تعارض\n\n"
                    "⚙️ <b>صلاحيات الإشراف (promoteChatMember):</b>\n"
                    "can_change_info, can_invite_users, can_pin_messages,\n"
                    "can_manage_topics, can_delete_messages, can_restrict_members,\n"
                    ...
                    "⚡ <b>الإعداد المسبق:</b>\n"
                    "• <code>full</code> — 👑 كامل الصلاحيات (بدون is_anonymous)\n\n"
                    "🛡 <b>منطق التطبيق:</b>\n"
                    "• <code>promoteChatMember</code> إذا كان أي perm = True\n"
                    "• إذا كل perms = False → تنزيل تلقائي صامت\n"
                    "• البوت يمنح فقط ما يملكه (intersection)\n"
                    "• بعد التطبيق: <code>get_chat_member</code> للتحقق + تسجيل التعارضات\n\n"
                    "⚠️ <b>حالات الخطأ المعالجة:</b>\n"
                    "• المستخدم غير موجود في المجموعة\n"
                    "• المستخدم مؤسس المجموعة\n"
                    "• المستخدم مشرف مجهول الهوية\n"
                    "• المُرسِل لا يملك can_promote_members\n"
                    "• البوت لا يملك can_promote_members\n"
                    "• @username غير موجود في قاعدة البيانات\n\n"
                    "📁 <b>الملفات:</b>\n"
                    "<code>handlers/group_admin/permissions_panel/</code>\n"
                    "  ├ handler.py       — نقطة الدخول + فحوصات الأمان\n"
                    "  ├ perms_config.py  — تعريف الصلاحيات والإعدادات المسبقة\n"
                    "  ├ perms_state.py   — إدارة الجلسة (StateManager)\n"
                    "  ├ perms_ui.py      — بناء الواجهة (2 أزرار/صف)\n"
                    "  ├ perms_apply.py   — تطبيق + تحقق post-apply\n"
                    "  └ perms_callbacks.py — معالجات الأزرار\n\n"
                    "👑 <b>الأوامر القديمة (لا تزال تعمل):</b>\n"
                    "رفع مشرف / تعديل مشرف / تنزيل مشرف\n"
                    "في: <code>handlers/group_admin/promote/promote_handler.py</code>"
                ),
            },
            {
